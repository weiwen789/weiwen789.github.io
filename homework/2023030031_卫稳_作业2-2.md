...
```bash
cat 1.gtf | awk '$1=="XI" && $3=="CDS"{print}' | sort -k5,5n | tail
```
```bash
...
```bash
grep -w '^IV' 1.gtf | awk '{print $3}' | sort | uniq -c | sort -k1,1n
```
```bash
    853 start_codon
    853 stop_codon
...
```bash
cat 1.gtf | awk '$1=="XV" && $3=="gene" {len=$5-$4+1;id=$10;gsub("\"","",id);gsub(";","",id);print id,len}'| sort -k2,2n | tail -5
```
```bash
YOR142W-B 5269
YOR192C-B 5314
...
统计1.gtf列数
```bash
grep -v '^#' 1.gtf | grep -v '^\s*$' | awk 'BEGIN{FS="\t"}{print NF}' | sort -n | uniq
```
```bash
9
```
